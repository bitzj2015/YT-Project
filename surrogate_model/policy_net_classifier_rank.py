...
class PolicyNetClassifier(torch.nn.Module):
    ...
    def forward(self, inputs, label, label_type, mask, with_graph=False):
        ...
        logits = torch.gather(F.softmax(outputs, -1), -1, label)

        reg_mat = torch.triu(torch.ones((label.size(-1), label.size(-1))))
        logits_reg = 1 - torch.matmul(logits, reg_mat)
        logits = torch.log(logits / (logits_reg + 1e-10) + 1e-10)
        # print(logits[0][0])
        ...
        print((mask * label_type * (rec_outputs == label)).sum() / (mask * label_type).sum())
