...
class RolloutWorker(object):
    ...
    def get_reward_gain(self):
        return self.cur_reward["removed"] + self.cur_reward["added"] - self.pre_reward["removed"] - self.pre_reward["added"]
...
class Env(object):
    ...
    def stop_env(self):
        self.clear_env()
        ray.get([worker.clear.remote() for worker in self.workers])
        ray.shutdown()
    ...
    def send_reward_to_workers(self):
        ...
        ray.get([self.workers[i].update_reward(cur_rec_base[i], cur_rec[i]) for i in range(len(self.workers))])
