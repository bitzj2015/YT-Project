...
if PHASE == 0:
    ...
    AVG_R = 0
    with open(f"../dataset/sock_puppets_rand_final_new2_cate_test4.json", "r") as json_file:
        data_truth = json.load(json_file)[2]["data"]

    with open(f"../dataset/video_metadata_rand_final_new2_cate_test4.json", "r") as json_file:
    ...
    for i in range(len(data_truth)):
        tmp[data_truth[i]["user_id"]] = data_truth[i]
    data_truth = tmp

    for i in range(N):
        try:
            base_data = data[f"base_{i}"]["homepage"]
            base_home_video_ids = {}
            base_cate = [0.001 for _ in range(len(CATE_IDS))]
            ...
            obfu_data = data[f"obfu_{i}"]["homepage"]
            obfu_home_video_ids = {}
            obfu_cate = [0.001 for _ in range(len(CATE_IDS))]
            ...
            delta_len = len(data[f"obfu_{i}"]["viewed"]) - len(data[f"base_{i}"]["viewed"])

            # print(sorted(base_home_video_ids))
            # print(sorted(obfu_home_video_ids))
            # seed_video = data[i]["viewed"][0]
            item_cate = [0.001 for _ in range(len(CATE_IDS))]
            item = data_truth[f"base_{i}"]
            # for item in data_truth:
            #     # if item["seedId"] == seed_video and item["viewed"][1] == data[i]["viewed"][1]:
            #     item = data_truth[i]
            #     flag = 1
            #     for k in range(len(item["viewed"])):
            #         if item["viewed"][k] != data[f"base_{i}"]["viewed"][k]:
            #             flag = 0
            #             break
            #     if flag == 1:
            item_home_video_ids = {}
            ...
            # print(i, item_cate)
            if delta_len > 6:
                AVG_R += np.sqrt(np.sum((np.array(base_cate) - np.array(obfu_cate)) ** 2)) # kl_divergence(base_cate, item_cate)
                # AVG_R += kl_divergence(base_cate, item_cate)
                # if base_cate[0] != item_cate[0]:
                #     print(base_cate, item_cate)
                #     print(item["viewed"], data[f"base_{i}"]["viewed"])
                #     break
                cnt += 1
                # fig, ax = plt.subplots()
                # plt.plot(base_cate)
