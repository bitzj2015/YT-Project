...
def run_regression_epoch(model, dataloader, mode="train", optimizer=None, ep=0, stat=None, logger=None, use_graph=False):
    for i, batch in tqdm(enumerate(dataloader)):
        ...
        last_cate = batch["last_cate"]
        logger.debug(input.size(), label.size(), label_type.size(), mask.size(), last_label.size(), last_label_type.size(), last_cate.size())
        ...
        if mode == "train":
            ...
            model.train()
            optimizer.zero_grad()
            loss_pos, loss_neg, last_acc, last_count, last_acc_ch = model(input, last_label, last_cate, last_label_type, mask, with_graph=use_graph)
