...
class PolicyNetClassifier(torch.nn.Module):
    def __init__(self, embed_dim, hidden_dim, video_embeddings, num_videos=127085):
    ...
    def forward(self, inputs, label, label_type, mask, with_graph=False):
        ...
        inputs = inputs * mask
        inputs = self.video_embeddings(inputs.reshape(-1).tolist(), with_graph).reshape(batch_size, seq_len, self.embed_dim)
            ...
            for j in range(sum(mask[i])):
                ...
                for k in range(num_rec):
                    if rec_outputs[i][j][k] in label_map.keys():
                        ...
                        if j == sum(mask[i]) - 1:
                            last_acc += 1
                            # print(rec_outputs[i][j][k])
                count += num_rec
                if j == sum(mask[i]) - 1:
                    last_count += num_rec
            #         print(num_rec)
            #         print(label[i][j][:num_rec])
            #         print(rec_outputs[i][j][:num_rec])
            # break
